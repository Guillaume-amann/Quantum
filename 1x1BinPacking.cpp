...
int run_qaoa_step(double gamma, double beta) {
    Qbit q; // Initialized to |0⟩ by default

    // 1. Hadamard to create superposition
    q = Gate::H().apply(q);

    // 2. Cost Hamiltonian: H_C = -2.5·I - 7.5·Z → U_C = Rz(-15γ)
    ...
    // 4. Measure the qubit
    return q.measure();  // 0 = empty, 1 = filled
}
...
int main() {
    ...
    cout << "Measurement result at optimal (γ, β): "
